    ...
    if (status && status.toUpperCase() === 'ACCEPTED') {
      ...
      if (tx.status !== 'COMPLETED') {
        ...
        try {
          ...
          // Envoyer l'email de confirmation de rechargement
          try {
            const [clientData] = await pool.execute('SELECT nom, prenom, email FROM clients WHERE id = ?', [clientId]);
            if (clientData.length > 0) {
              emailService.sendRechargeSuccessEmail(clientData[0], transaction_id, tx.amount, newBalance);
            }
          } catch (emailErr) {
            console.error('Erreur envoi email succès rechargement:', emailErr);
          }
      ...
      // Envoyer un email d'échec si le statut est REFUSED ou similaire
      if (newStatus === 'REFUSED' || newStatus === 'FAILED' || newStatus === 'CANCELLED') {
        try {
          const [clientData] = await pool.execute('SELECT nom, prenom, email FROM clients WHERE id = ?', [clientId]);
          if (clientData.length > 0) {
            emailService.sendRechargeFailedEmail(clientData[0], transaction_id, tx.amount);
          }
        } catch (emailErr) {
          console.error('Erreur envoi email échec rechargement:', emailErr);
        }
      }
